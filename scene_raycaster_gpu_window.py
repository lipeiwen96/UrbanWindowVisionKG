...
LABEL_MAP = {
    ...
    2: "road",
    3: "water",
    4: "skyscraper_structure", # 修改: 明确是结构
    5: "tree",
    6: "mountain",
    SKY_LABEL: "sky",
...
# --- 几何体创建函数 ---
# *** 修改: create_box 现在生成墙体和窗户 ***
def create_box_with_windows(center, size,
                            window_ratio_x=0.6, window_ratio_y=0.4,
                            window_spacing_x=0.1, window_spacing_y=0.2):
    """
    创建带窗户的长方体。
    返回: verts (所有顶点), faces_dict (包含 'wall', 'window', 'top', 'bottom' 面列表的字典)
    """
    cx, cy, cz = center
    ...
    # 1. 定义长方体的 8 个角点
    verts = np.array([
        [cx - sx, cy - sy, cz - sz], [cx + sx, cy - sy, cz - sz], # 0, 1 (底面前)
        [cx + sx, cy + sy, cz - sz], [cx - sx, cy + sy, cz - sz], # 2, 3 (顶面前)
        [cx - sx, cy - sy, cz + sz], [cx + sx, cy - sy, cz + sz], # 4, 5 (底面后)
        [cx + sx, cy + sy, cz + sz], [cx - sx, cy + sy, cz + sz], # 6, 7 (顶面后)
    ], dtype=GEOMETRY_DTYPE)
    num_base_verts = 8

    faces_dict = {'wall': [], 'window': [], 'top': [], 'bottom': []}

    # 2. 添加顶部和底部面 (属于 'wall' 或 'building' 类别)
    faces_dict['bottom'] = [(0, 1, 5), (0, 5, 4)] # 底面
    faces_dict['top'] = [(3, 7, 6), (3, 6, 2)] # 顶面
    ...
    current_vert_idx = num_base_verts
    all_verts_list = list(verts) # 开始构建顶点列表

    for v0_idx, v1_idx, v2_idx, v3_idx in side_indices:
        # 获取侧面的四个角点
        p0 = verts[v0_idx] # 左下
        p1 = verts[v1_idx] # 右下
        p2 = verts[v2_idx] # 右上
        ...
        # 计算侧面的宽度和高度
        side_width_vec = p1 - p0
        side_height_vec = p3 - p0
        side_width = np.linalg.norm(side_width_vec)
        side_height = np.linalg.norm(side_height_vec)

        # 如果侧面太小，则不添加窗户
        if side_width < 1e-3 or side_height < 1e-3:
            # 将整个侧面添加为墙体
            faces_dict['wall'].extend([(v0_idx, v1_idx, v2_idx), (v0_idx, v2_idx, v3_idx)])
            continue

        # 计算窗户尺寸和间距
        win_width = side_width * window_ratio_x
        win_height = side_height * window_ratio_y
        space_x = side_width * window_spacing_x
        space_y = side_height * window_spacing_y

        # 计算可以放置多少个窗户
        num_win_x = max(1, int((side_width - space_x) / (win_width + space_x)))
        num_win_y = max(1, int((side_height - space_y) / (win_height + space_y)))

        # 实际使用的间距和窗户尺寸，使其均匀分布
        total_win_width = num_win_x * win_width
        total_space_x = side_width - total_win_width
        actual_space_x = total_space_x / (num_win_x + 1)

        total_win_height = num_win_y * win_height
        total_space_y = side_height - total_win_height
        actual_space_y = total_space_y / (num_win_y + 1)

        # 侧面的归一化方向向量
        side_width_dir = side_width_vec / side_width
        side_height_dir = side_height_vec / side_height
        ...
        # 迭代生成窗户和墙体网格
        last_y = 0.0
        for iy in range(num_win_y + 1):
            current_y_bottom = last_y + actual_space_y
            current_y_top = current_y_bottom + win_height if iy < num_win_y else side_height

            last_x = 0.0
            for ix in range(num_win_x + 1):
                current_x_left = last_x + actual_space_x
                current_x_right = current_x_left + win_width if ix < num_win_x else side_width

                # 计算当前单元格的四个角点
                v00 = p0 + last_x * side_width_dir + last_y * side_height_dir
                v10 = p0 + current_x_right * side_width_dir + last_y * side_height_dir
                v01 = p0 + last_x * side_width_dir + current_y_top * side_height_dir
                v11 = p0 + current_x_right * side_width_dir + current_y_top * side_height_dir

                # 添加顶点到列表 (避免重复添加相同的角点)
                # (更健壮的方法是使用字典映射坐标到索引，但这里为了简化，每次都添加)
                idx00 = current_vert_idx
                all_verts_list.append(v00)
                current_vert_idx += 1
                idx10 = current_vert_idx
                all_verts_list.append(v10)
                current_vert_idx += 1
                idx01 = current_vert_idx
                all_verts_list.append(v01)
                current_vert_idx += 1
                idx11 = current_vert_idx
                all_verts_list.append(v11)
                current_vert_idx += 1

                # 判断是窗户还是墙体
                is_window = (ix < num_win_x and iy < num_win_y)

                if is_window:
                    # 添加窗户面 (两个三角形)
                    side_window_faces.extend([(idx00, idx10, idx11), (idx00, idx11, idx01)])
                else:
                    # 添加墙体面 (两个三角形)
                    side_wall_faces.extend([(idx00, idx10, idx11), (idx00, idx11, idx01)])

                last_x = current_x_right

            last_y = current_y_top

        # 将此侧面的墙体和窗户面添加到总列表中
        faces_dict['wall'].extend(side_wall_faces)
        faces_dict['window'].extend(side_window_faces)


    final_verts = np.array(all_verts_list, dtype=GEOMETRY_DTYPE)
    return final_verts, faces_dict
...
def create_prism(center, height, radius, sides=5):
    ...
    # 对于棱柱，我们暂时不添加窗户，将其归类为 'skyscraper_structure'
    return verts, {'structure': faces} # 返回字典以保持一致性

# *** 修改: create_skyscraper 现在生成结构和窗户 ***
def create_skyscraper_with_windows(center, base_width, height, levels=8, taper=0.85,
                                   floor_height_ratio=0.1, mullion_width_ratio=0.05):
    """
    创建多层、带窗户的摩天楼 (落地玻璃 + 楼板)。
    返回: verts, faces_dict ('structure', 'window')
    """
    verts_list = []
    faces_dict = {'structure': [], 'window': []}
    cx, cy_base, cz = center
    current_y = cy_base
    current_width = base_width
    total_segment_height = height / float(levels)
    floor_slab_height = total_segment_height * floor_height_ratio
    window_height = total_segment_height - floor_slab_height
    mullion_width = current_width * mullion_width_ratio # 基于当前层的宽度

    current_vert_idx = 0

    for level in range(levels):
        level_base_y = current_y
        level_top_y = current_y + total_segment_height
        half_width = current_width / 2.0
        next_width = current_width * taper
        next_half_width = next_width / 2.0

        # --- 定义当前层和下一层的角点 ---
        # 当前层底部
        p000 = (cx - half_width, level_base_y, cz - half_width)
        p100 = (cx + half_width, level_base_y, cz - half_width)
        p101 = (cx + half_width, level_base_y, cz + half_width)
        p001 = (cx - half_width, level_base_y, cz + half_width)
        # 当前层顶部 (窗户底部)
        p010_w = (cx - half_width, level_base_y + floor_slab_height, cz - half_width)
        p110_w = (cx + half_width, level_base_y + floor_slab_height, cz - half_width)
        p111_w = (cx + half_width, level_base_y + floor_slab_height, cz + half_width)
        p011_w = (cx - half_width, level_base_y + floor_slab_height, cz + half_width)
        # 下一层底部 (当前层顶部 / 楼板顶部)
        p010_next = (cx - next_half_width, level_top_y, cz - next_half_width)
        p110_next = (cx + next_half_width, level_top_y, cz - next_half_width)
        p111_next = (cx + next_half_width, level_top_y, cz + next_half_width)
        p011_next = (cx - next_half_width, level_top_y, cz + next_half_width)


        # --- 创建楼板 (一个薄的长方体) ---
        # 楼板顶点
        floor_verts_indices = []
        floor_verts = [p000, p100, p101, p001, p010_w, p110_w, p111_w, p011_w]
        for v in floor_verts:
            verts_list.append(v)
            floor_verts_indices.append(current_vert_idx)
            current_vert_idx += 1

        # 楼板面 (侧面 + 顶面，底面是上一层的顶面) - 属于 'structure'
        fv = floor_verts_indices
        faces_dict['structure'].extend([
            (fv[0], fv[1], fv[5]), (fv[0], fv[5], fv[4]), # 前
            (fv[1], fv[2], fv[6]), (fv[1], fv[6], fv[5]), # 右
            (fv[2], fv[3], fv[7]), (fv[2], fv[7], fv[6]), # 后
            (fv[3], fv[0], fv[4]), (fv[3], fv[4], fv[7]), # 左
            (fv[4], fv[5], fv[6]), (fv[4], fv[6], fv[7])  # 顶 (窗户底部)
        ])

        # --- 创建窗户和竖框 (Mullions) ---
        # 侧面定义 (当前层窗户底部 到 下一层楼板底部)
        side_defs = [
            (p010_w, p110_w, p110_next, p010_next), # 前 (-Z)
            (p110_w, p111_w, p111_next, p110_next), # 右 (+X)
            (p111_w, p011_w, p011_next, p111_next), # 后 (+Z)
            (p011_w, p010_w, p010_next, p011_next)  # 左 (-X)
        ]

        for p_bl, p_br, p_tr_next, p_tl_next in side_defs: # 底左, 底右, (下一层)顶右, (下一层)顶左
            # 计算侧面向量
            side_vec = np.array(p_br) - np.array(p_bl)
            side_len = np.linalg.norm(side_vec)
            side_dir = side_vec / side_len if side_len > 1e-6 else np.array([0,0,0])

            # 计算竖框的偏移量
            mullion_offset_vec = side_dir * (mullion_width / 2.0)

            # 定义竖框和窗户的顶点
            # 左竖框顶点
            v00_ml = p_bl
            v10_ml = p_bl + mullion_offset_vec
            v01_ml = p_tl_next # 使用下一层的点来处理锥度
            v11_ml = np.array(p_tl_next) + mullion_offset_vec * (next_width / current_width) # 调整竖框偏移以适应锥度

            # 窗户顶点
            v00_w = v10_ml
            v10_w = p_br - mullion_offset_vec
            v01_w = v11_ml
            v11_w = np.array(p_tr_next) - mullion_offset_vec * (next_width / current_width)
            ...
            # 添加顶点到列表并获取索引
            verts_to_add = [
                v00_ml, v10_ml, v01_ml, v11_ml, # 左竖框
                v00_w, v10_w, v01_w, v11_w,   # 窗户
                v00_mr, v10_mr, v01_mr, v11_mr  # 右竖框
            ]
            indices = []
            for v in verts_to_add:
                verts_list.append(v)
                indices.append(current_vert_idx)
                current_vert_idx += 1

            # 添加面
            # 左竖框 (structure)
            ml_i = indices[0:4]
            faces_dict['structure'].extend([(ml_i[0], ml_i[1], ml_i[3]), (ml_i[0], ml_i[3], ml_i[2])])
            # 窗户 (window)
            w_i = indices[4:8]
            faces_dict['window'].extend([(w_i[0], w_i[1], w_i[3]), (w_i[0], w_i[3], w_i[2])])
            # 右竖框 (structure)
            mr_i = indices[8:12]
            faces_dict['structure'].extend([(mr_i[0], mr_i[1], mr_i[3]), (mr_i[0], mr_i[3], mr_i[2])])


        # --- 如果是最后一层，添加顶盖 ---
        if level == levels - 1:
            # 使用下一层的点作为顶盖顶点
            top_cap_verts_indices = []
            top_cap_verts = [p010_next, p110_next, p111_next, p011_next]
            for v in top_cap_verts:
                verts_list.append(v)
                top_cap_verts_indices.append(current_vert_idx)
                current_vert_idx += 1
            # 添加顶盖面 (structure)
            tcvi = top_cap_verts_indices
            faces_dict['structure'].extend([(tcvi[0], tcvi[1], tcvi[2]), (tcvi[0], tcvi[2], tcvi[3])])


        # 为下一层更新
        current_y = level_top_y
        current_width = next_width
        mullion_width = current_width * mullion_width_ratio # 更新竖框宽度

    final_verts = np.asarray(verts_list, dtype=GEOMETRY_DTYPE)
    return final_verts, faces_dict

# (create_cylinder 基本不变)
...
def build_scene():
    ...
    target_num_prisms = 500 # 棱柱暂时不加窗户
    ...
    def add_object(verts, faces, label, color_map):
        ...
        for face_indices in faces:
            ...
            if len(face_indices) == 3: # 只处理三角形
                a, b, c = face_indices
                if a < num_verts and b < num_verts and c < num_verts:
                    # 为此三角形附加顶点、标签和颜色
                 ...
                 invalid_count += 1 # 非三角形面
        # if invalid_count > 0:
        #     print(f"警告: 标签 {label} 中跳过了 {invalid_count} 个无效或非三角形面。添加了 {added_count} 个。")
    ...
    # *** 使用普通 create_box 创建地面 ***
    g_v, g_f_dict = create_box_with_windows(ground_center, ground_size, window_ratio_x=0, window_ratio_y=0) # 无窗户
    add_object(g_v, g_f_dict['wall'] + g_f_dict['top'] + g_f_dict['bottom'], 0, C) # 标签 0: ground
    ...
    # *** 使用普通 create_box 创建湖泊 ***
    lake_v, lake_f_dict = create_box_with_windows(lake_center, lake_size, window_ratio_x=0, window_ratio_y=0) # 无窗户
    # 只添加湖泊长方体的顶面
    add_object(lake_v, lake_f_dict['top'], 3, C) # 标签 3: water
    # 定义湖泊边界用于碰撞检查
    ...
    for x in grid_coords:
        v, f_dict = create_box_with_windows((x, road_y, 0.0), (road_w, road_h, road_len), 0, 0)
        add_object(v, f_dict['wall'] + f_dict['top'] + f_dict['bottom'], 2, C) # 标签 2: road
    # 沿 Z 轴添加道路
    for z in grid_coords:
        v, f_dict = create_box_with_windows((0.0, road_y, z), (road_len, road_h, road_w), 0, 0)
        add_object(v, f_dict['wall'] + f_dict['top'] + f_dict['bottom'], 2, C) # 标签 2: road
    ...
    while building_count < target_num_buildings and attempts < max_attempts_building:
        ...
        # *** 使用 create_box_with_windows ***
        win_rx = RAND.uniform(0.4, 0.8)
        win_ry = RAND.uniform(0.3, 0.6)
        win_sx = RAND.uniform(0.05, 0.15)
        win_sy = RAND.uniform(0.1, 0.25)
        v, f_dict = create_box_with_windows((x, center_y, z), (w, h, w),
                                            window_ratio_x=win_rx, window_ratio_y=win_ry,
                                            window_spacing_x=win_sx, window_spacing_y=win_sy)
        # 分别添加墙体和窗户
        add_object(v, f_dict['wall'] + f_dict['top'] + f_dict['bottom'], 1, C) # 标签 1: building_wall
        add_object(v, f_dict['window'], WINDOW_LABEL, C) # 标签 8: window
        building_count += 1
    ...
    while skyscraper_count < target_num_skyscrapers and attempts < max_attempts_skyscraper:
        ...
        floor_hr = RAND.uniform(0.08, 0.15)
        mullion_wr = RAND.uniform(0.04, 0.08)
        # *** 使用 create_skyscraper_with_windows ***
        v, f_dict = create_skyscraper_with_windows((x, base_y, z), base_w, h,
                                                   levels=levels, taper=taper,
                                                   floor_height_ratio=floor_hr,
                                                   mullion_width_ratio=mullion_wr)
        # 分别添加结构和窗户
        add_object(v, f_dict['structure'], 4, C) # 标签 4: skyscraper_structure
        add_object(v, f_dict['window'], WINDOW_LABEL, C) # 标签 8: window
        skyscraper_count += 1
    ...
    while prism_count < target_num_prisms and attempts < max_attempts_prism:
        ...
        center_y = base_y + h / 2.0
        v, f_dict = create_prism((x, center_y, z), h, r, sides=sides)
        # 将整个棱柱添加为 'structure'
        add_object(v, f_dict['structure'], 4, C) # 对这些特殊建筑也使用标签 4
        prism_count += 1
...
def save_combined_obj(filename, v0s, e1s, e2s, normals, labels, # 原始几何数据
    ...
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            # 定义视锥体边缘的线
            if len(corner_indices) == 4: # 确保计算了所有角点
                bl, tl, br, tr = corner_indices # 左下, 左上, 右下, 右上 (顺序可能依赖于循环)
                # 假设顺序是: [-0.5,-0.5], [-0.5,0.5], [0.5,-0.5], [0.5,0.5] -> bl, tl, br, tr
                bl, tl, br, tr = corner_indices[0], corner_indices[1], corner_indices[2], corner_indices[3]
